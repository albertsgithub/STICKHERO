...
public class Mojing3rdCamera : MonoBehaviour 
{
	...
	private Transform cardboardMain;
...
//	private bool snap = false;
	...
	void Awake ()
	{	
		cardboardMain = GameObject.Find("MojingMain").transform;
		if(!cardboardMain && Camera.main)
			cardboardMain = Camera.main.transform;
		if(!cardboardMain) 
		{
	...
	void Apply (Transform dummyTarget, Vector3 dummyCenter)
	{
		...
		float currentAngle = cardboardMain.eulerAngles.y;
		...
		// Damp the height
		float currentHeight = cardboardMain.position.y;
		...
		cardboardMain.position = targetCenter;
		cardboardMain.position += currentRotation * Vector3.back * distance;

		// Set the height of the camera
		cardboardMain.position = new Vector3(cardboardMain.position.x, currentHeight,cardboardMain.position.z);
